...
function Invoice() {
  ...
  const addPayment = () => {
    setPayments([...payments, { amount: "", method: "" }]);
  };
  ...
  const handleEdit = (r) => {
    ...
    setRows(r.rows || [{ treatment: "", doctor: "", qty: "", rate: "" }]);
    setPayments(r.payments || [{ amount: "", method: "" }]);
    setDiscount(r.discount || "");
    setLabCharge(r.lab_charge || "");
    setEditId(r._id);
  };
  ...
  const saveInvoice = async () => {
    try {
      ...
      if (editId) {
        await api.put("/invoice/" + editId, payload);
      } else {
  ...
  return (
    ...
    <Layout>
      ...
      <h1 style={{ marginBottom: 20 }}>Invoice System 🧾</h1>
      ...
      <div style={card}>
        ...
        {records.map(r => (
          <div key={r._id} style={{
            ...
            <button onClick={() => deleteInvoice(r._id)}>Delete ❌</button>
          </div>
        ))}
      </div>
